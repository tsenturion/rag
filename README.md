...
- долговременная SQLite memory хранит user-scoped и session-scoped факты, теги, importance, TTL и счётчик обращений;
...
rag-agent `
  --config config/multi_agent_openai.yaml `
  --compare-agents `
  --multi-agent-scenarios config/multi_agent_module2_scenarios.yaml `
  --user-id module-2 `
  --json
```
