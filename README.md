...
`local_device: auto` выбирает XPU, затем CUDA и затем CPU. `max_input_tokens`
ограничивается также контекстным окном модели с резервом под `max_new_tokens`.
`trust_remote_code` по умолчанию выключен; значение `true` включайте только после
проверки и фиксации revision кода репозитория модели. Если используется LoRA/PEFT
...
Пустой входной корпус и нулевой результат чанкинга всегда считаются ошибкой независимо
от `fail_on_validation_error`: успешный manifest не может описывать отсутствующий RAG-корпус.
Неизвестные поля запрещены во всех вложенных секциях конфигурации, поэтому опечатка вроде
`chunk_szie` останавливает запуск до чтения данных.
...
```powershell
rag-agent --config config/agent_openai.yaml --user-id default --session-id lesson-1 --message "Запомни, что мой проект называется RAG Engineer Assistant"
```

Посмотреть память:

```powershell
rag-agent --config config/agent_openai.yaml --user-id default --list-memory
```

Очистить память текущей сессии:

```powershell
rag-agent --config config/agent_openai.yaml --user-id default --session-id default --clear-session-memory
```
...
Перейдите в корень клонированного репозитория. Например:
...
Ограничение частоты запросов на host по умолчанию использует очищаемый in-memory
token bucket и рассчитано на один процесс. Docker-профиль
`config/profiles/support/deployment_docker.yaml` переключает
`security.rate_limit_backend` на Redis из `ORCHESTRATION_REDIS_URL`: Lua-операция
атомарно списывает квоту, поэтому несколько Uvicorn replicas видят общий лимит.
Redis-режим не деградирует молча до памяти: при недоступном Redis сервис не стартует.
...
Аналогично `GetTask`, `ListTasks` и `CancelTask` не раскрывают чужие задачи. Суммарный
текст всех message parts проверяется по `service.request_max_chars` до сохранения задачи.
Отмена разрешена только в состоянии `submitted`: Python не может безопасно остановить
уже начатый синхронный provider/tool-вызов в другом потоке, поэтому попытка отмены
`working`-задачи возвращает unsupported operation и не скрывает продолжающиеся расходы
или побочные действия ложным статусом `cancelled`.
...
После сохранения показанной выше секции в собственный конфиг можно
детерминированно вызвать внешний tool без LLM:

```powershell
rag-agent `
  --config <config-with-company-mcp.yaml> `
...
`1.28.1` и MLflow `3.15.2`. Переход на Starlette 1.x устраняет известные уязвимости
...
На 3 сентября 2026 года NLTK `3.10.3` является последним опубликованным релизом, но
[GHSA-8mgp-746c-j5xp](https://github.com/nltk/nltk/security/advisories/GHSA-8mgp-746c-j5xp)
не имеет исправленной версии. Advisory относится к обходу `pathsec` в API импорта и
экспорта model artifacts: `TransitionParser`, `AveragedPerceptron`,
`PerceptronTagger` и maxent persistence. Проект получает NLTK транзитивно через
LlamaIndex, фиксирует минимальную версию `3.10.3` и не импортирует эти API. Поэтому
полный data-pipeline audit временно исключает только `PYSEC-2026-3740`; service и
code-runner requirements проверяются без этого исключения. После публикации
исправленного NLTK ограничение версии и список исключений должны быть обновлены
одновременно.
...
На Linux direct-запуск возможен в отдельном терминале:
...
На Windows direct execution отключён: платформа не предоставляет применяемые проектом
POSIX resource limits для памяти, CPU, файлов и процессов. Не включайте внутреннюю
переменную test-only обхода; поддерживаемый Windows-сценарий запускает `code-runner` и
`support-agent` через Docker Compose, как показано ниже. AST allowlist является
дополнительным фильтром, но не заменяет контейнерную границу ресурсов.
...
Штатный отдельный worker запускается в Compose-сети, где адреса RabbitMQ/Redis и
credentials формируются из одного `.env`. Host-конфиг `multi_agent_openai.yaml` имеет
`backend: inline` и для отдельного worker не подходит:

```powershell
docker compose --profile orchestration up -d orchestration-worker
```

Прямой CLI `rag-orchestration-worker --config <docker-provider-config>` нужен только
для host-отладки. Перед ним `ORCHESTRATION_BROKER_URL`,
`ORCHESTRATION_RESULT_BACKEND_URL` и `ORCHESTRATION_REDIS_URL` должны указывать на
`127.0.0.1`; пароль в broker URL обязан совпадать с `RABBITMQ_DEFAULT_PASS`.
...
- `fact_precision`, `fact_recall`, `fact_f1` - покрытие ожидаемых фактов и доля утверждений ответа, имеющих опору в `expected_facts`;
- `unsupported_claim_rate` - доля запрещённых или дополнительных утверждений без опоры в `expected_facts`; сами фрагменты сохраняются в `unsupported_claims`;
...
Это консервативная детерминированная проверка, а не LLM-as-a-judge. Дополнительное
утверждение не объявляется ложным, но считается неподтверждённым и уменьшает precision.
Поэтому критические кейсы должны содержать достаточно полный набор `expected_facts` и
`forbidden_facts`; для субъективной оценки остаётся human review. Каждый evaluation run
получает отдельные `user_id` и `session_id`, чтобы память предыдущего запуска не влияла
на последующие измерения.
...
Prometheus загружает правила `SupportAgentHighErrorRate`, `SupportAgentHighP95Latency`, `SupportAgentPromptInjectionDetected` и `SupportAgentHumanReviewBacklog`. Метрика `support_agent_human_reviews_pending` вычисляется по фактическому числу незакрытых записей review-store при чтении `/metrics`, поэтому backlog уменьшается после решения и не является числом новых событий за окно времени. Локальный Alertmanager использует пустой receiver и предназначен для демонстрации; production receiver для email, PagerDuty или webhook задаётся организацией отдельно.
...
в `[tool.coverage.run]` и `[tool.coverage.report]` файла `pyproject.toml`. Coverage
измеряет весь каталог `src` без исключения CLI, LLM, Celery tasks, generation,
training или pipeline-модулей. Тяжёлые внешние границы подменяются контролируемыми
адаптерами, но тесты исполняют provider routing, batching/pooling embeddings,
tokenization, generation/evaluation/training lifecycle, pipeline orchestration,
`rag-index`, `llm-tune`, code-runner CLI и Celery worker task. Реальные
OpenAI/GigaChat/OpenWeather/Hugging Face интеграции остаются в ручном workflow
`live-api.yaml`, чтобы обычный CI не расходовал API-квоту.
