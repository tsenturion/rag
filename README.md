...
Docker Compose создаёт три взаимодействующих сервиса:
...
- `.github/workflows/quality.yaml` запускается для pull request, push в `main` и вручную. Он проверяет зависимости, Ruff, агентные и оркестрационные тесты, сохраняет JUnit-отчёт, собирает Docker-образы и выполняет реальный HTTP smoke test контейнеров;
...
Реальные интеграции проверяются отдельно, чтобы pull request или автоматический push не создавали расходы. Вы выбрали правильный тип секретов: откройте репозиторий GitHub → **Settings → Secrets and variables → Actions → Repository secrets** и добавьте значения с точными именами:
...
После commit и push workflow появится во вкладке **Actions → Проверка реальных API → Run workflow**. Параметр `target` выбирает объём проверки: `core` вызывает OpenAI, GigaChat и OpenWeatherMap; отдельные значения вызывают один API; `all` дополнительно проверяет `HF_TOKEN`. Тот же безопасный скрипт можно выполнить локально:
...
Итоговая аттестация не образует отдельный программный модуль: она использует тот же support-сервис, сценарии, evaluation pipeline, артефакты, метрики и контейнеры, которые применяются при эксплуатации. Проверяемый маршрут состоит из следующих этапов:

1. Выбрать явный provider preset и объяснить роли, разрешённые tools, embedding profile, orchestration pattern и ограничения в YAML.
2. Поднять Qdrant, support-agent, code runner, RabbitMQ/Redis и observability через Docker Compose; для BPMN дополнительно включить профиль `bpmn`.
3. Проверить `/ready`, выполнить основной, альтернативный и ошибочный запросы через Swagger или Postman, затем отправить orchestration jobs с паттернами `sequential`, `parallel`, `conditional`, `quorum` и `dynamic`.
4. Запустить `rag-eval` на `config/evaluation/engineering_support_cases.yaml` и показать `report.json`, `results.jsonl`, MLflow run, citations, latency, стоимость и quality gates.
5. В Jaeger показать связанный trace, в Grafana - latency/error/guardrail панели, в Flower и RabbitMQ Management - worker и очереди, в security audit - отклонённый prompt injection.
6. Зафиксировать ограничения и план развития: внешняя БД для нескольких API replicas, secret manager, TLS, provider moderation/DLP, production Alertmanager receiver и canary deployment immutable image digest.

Минимальный набор команд для демонстрации:

```powershell
$env:SUPPORT_AGENT_CONFIG = "config/multi_agent_docker_openai_observability.yaml"
$env:VECTOR_STORE_CONFIG = "config/vector_store_docker_openai.yaml"

docker compose --profile indexing run --rm indexer
docker compose --profile observability --profile orchestration up -d --build
...
Артефактами защиты являются immutable исходный commit/tag, provider-конфигурация, `data/evaluation/runs/<run_id>/`, multi-agent run directory, MLflow run, JUnit artifact из CI и ссылки на trace/dashboard. Секреты, `.env`, SQLite runtime data и содержимое memory в артефакты не включаются.

## Матрица покрытия программы

| Требование | Исполняемая реализация | Проверка или артефакт |
|---|---|---|
| Архитектура, роли, цели и декомпозиция | LangGraph supervisor, role registry, rule/hybrid/LLM planner, per-role LLM routing | `multi_agent/graph.py`, `decomposition.py`, `roles.py`, comparison report |
| Жизненный цикл и граф состояний | lifecycle events, checkpoints, bounded rounds/delegations/token budget | `lifecycle.py`, `persistence.py`, `test_multi_agent_core.py` |
| A2A, MCP, legacy ACP, request-response и pub-sub | A2A SDK routes, собственный и внешний MCP, ACP-to-A2A adapter, async message bus | Agent Card, `/a2a`, `/mcp/`, protocol tests |
| Память и контекст | short-term buffer, summary memory, user/session-scoped SQLite memory с TTL, Qdrant retrieval | memory tests, `retrieval.status`, citations |
| Tools и инфраструктурные интеграции | OpenWeatherMap, calculator, incidents, filesystem sandbox, isolated code runner, MCP tools | tool tests, `/health`, code-runner container |
| Последовательная, параллельная и условная оркестрация | LangGraph orchestration engine и пять ограниченных patterns | orchestration tests и job events |
| Синхронизация, кворум и адаптация | dependency barrier, quorum/consensus, dynamic replanning и fallback roles | `synchronization.py`, `planning.py`, dynamic/quorum tests |
| Очереди, приоритеты и нагрузка | Celery, RabbitMQ priority/DLQ, Redis state, idempotency, backpressure, provider limits | Flower, RabbitMQ Management, `/metrics`, orchestration tests |
| BPMN + agents | исполняемый Camunda process и job workers | `engineer_support.bpmn`, Camunda deploy/start/status |
| Безопасность и приватность | API key/JWT, RBAC, user isolation, prompt/context/output guardrails, audit, human review | security tests, audit API, review queue |
| Качество и стоимость | фиксированный dataset, precision/recall/F1, consistency, latency, provider tariffs, quality gates | `rag-eval`, JSON/JSONL manifest, MLflow |
| Наблюдаемость | JSON logs, OpenTelemetry, Jaeger, Prometheus, Alertmanager и Grafana | trace IDs, targets `up`, dashboard и alert rules |
| Контейнеризация и масштабирование | два Dockerfile, Compose profiles, stateless Celery workers и `docker compose --scale` | Docker healthchecks и CI smoke test |
| Тестирование и CI/CD | unit/property/regression/simulation tests, pinned Actions, GHCR release с SBOM/provenance | GitHub checks, JUnit artifact и published image digest |

В практической реализации сознательно выбран один вариант из допустимых альтернатив: LangGraph вместо AutoGen, RabbitMQ вместо Kafka и Jaeger вместо Tempo. Они закрывают требуемые паттерны; замена фреймворка, broker или trace backend является отдельной миграцией, а не условием одновременного запуска всех перечисленных технологий. API replicas пока не масштабируются горизонтально из-за локальных SQLite-хранилищ; горизонтально масштабируются orchestration workers. Это ограничение явно учитывается в плане production-развития.

Официальные материалы: [OpenTelemetry Python](https://opentelemetry.io/docs/languages/python/instrumentation/), [OpenTelemetry Collector](https://opentelemetry.io/docs/collector/), [OTLP exporter configuration](https://opentelemetry.io/docs/specs/otel/protocol/exporter/), [OWASP Prompt Injection Prevention](https://cheatsheetseries.owasp.org/cheatsheets/LLM_Prompt_Injection_Prevention_Cheat_Sheet.html), [Prometheus alerting](https://prometheus.io/docs/alerting/latest/overview/), [Alertmanager](https://prometheus.io/docs/alerting/latest/alertmanager/), [Jaeger](https://www.jaegertracing.io/docs/latest/), [модель и тариф GPT-5.4 nano](https://developers.openai.com/api/docs/models/gpt-5.4-nano), [тарифы GigaChat для физлиц](https://developers.sber.ru/docs/ru/gigachat/tariffs/individual-tariffs) и [тарифы GigaChat для юрлиц](https://developers.sber.ru/docs/ru/gigachat/tariffs/legal-tariffs).
